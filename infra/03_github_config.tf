...
resource "github_actions_environment_variable" "vars" {
  for_each = {
    for key, value in {
      ...
      "NEON_DB_BRANCH_NAME"                      = local.neon_effective_branch_name
      ...
      "OIDC_AUDIENCE"                            = local.oidc_audience_value
      ...
      "NEXT_PUBLIC_KEYCLOAK_URL"                 = local.keycloak_base_url
      "NEXT_PUBLIC_KEYCLOAK_REALM"               = local.keycloak_realm
      "NEXT_PUBLIC_KEYCLOAK_CLIENT_ID"           = local.auth_frontend_client_id
      "NEXT_PUBLIC_KEYCLOAK_SCOPE"               = var.next_public_auth_scope
      "NEXT_PUBLIC_KEYCLOAK_REDIRECT_URI"        = local.client_redirect_uri
      "NEXT_PUBLIC_KEYCLOAK_LOGOUT_REDIRECT_URI" = local.client_logout_uri
