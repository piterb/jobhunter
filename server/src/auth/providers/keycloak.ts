...
export class KeycloakAdapter implements AuthProviderAdapter {
    ...
    constructor(config: OidcAdapterConfig) {
        ...
        this.jwksUrl = `${issuer}/protocol/openid-connect/certs`;
    }
    ...
    async authenticate(token: string): Promise<AuthContext> {
        try {
            ...
            const realmAccess = (payload as JwtPayloadLike & { realm_access?: RealmAccess }).realm_access;
            const resourceAccess = (payload as JwtPayloadLike & { resource_access?: ResourceAccess }).resource_access;
            const resourceRoles = Object.values(resourceAccess || {}).flatMap((entry) => normalizeStringArray(entry.roles));

            const roles = mergeUnique([
                normalizeStringArray(payload.roles),
                normalizeStringArray(realmAccess?.roles),
                resourceRoles
            ]);
